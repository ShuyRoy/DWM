...
# 进行双边单边循环
def sumShift1(place,sch,access,PM,shift):
    tmp_shift = shift
    frequency = 0
    place11 = []
    place0 = copy.deepcopy(place)
    while frequency <= 3*PM:
        place1 = dataPlacement(place0, PM, frequency)
        tmp_sch = copy.deepcopy(sch)
        temp = 0    # 记录shift次数
        index = 0   # port当前位置
        a = 0   # 从sch中的第0个位置开始找
        i = 0  # 记录步数
        ...
        if temp < tmp_shift:
            tmp_shift = temp
            place11 = copy.deepcopy(place1)
        frequency += 1
    return tmp_shift,place11
...
def sumShift2(sch,access,PM,shift,place):
    tmp_shift = shift
    tmp_sch = sch[:]
    frequency = 0
    shift00 = []
...
if __name__ == "__main__":
    ...
    file_path = "D:\研究生学习\研究生基础知识储备\数据存储讨论班\论文\exp-1-puremacro//ghostscript"
    ...
    tmp = []
    sides = []
    with open(file_path + "//ghostscript4.txt") as file_object:
        for line in file_object:
    ...
    place = [-1] * P
    port_p = 0
    shift1 = greedy(nodes, sides, sch, place, port_p, PM)
    ...
    shift2,place1 = sumShift1(temp_place,sch,access,PM,shift1)
    newShift,place2 = sumShift2( sch, access, PM, shift2,place1)
    print("newShift",newShift,shift2,shift1)

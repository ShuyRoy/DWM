...
def sumShift2(v,e,sch,shift):        # 计算总的shift距离
    list0,list1,list2 = indegree0(v, e)
    tmp_sch = []
    newShift,place0 = sumShift(sch, access, PM, shift)
    tmp_shift = newShift
    ...
    while list0:
        ...
        while index + i < PM:
            for a in v:
                if tmp_place[index + i] == access[a] or tmp_place[index + i + PM] == access[a] and a not in sch:
                    tmp_sch.append(a)                       # 保留此次访问顺序
                    change_list02(tmp_sch[-1], e, list0, list2)
                    find_same(tmp_sch, access, e, list0, list2)
                    index += i                                              # 指示目前port的位置
                    temp += i                                               # 记录本次移动的距离
                    # i = 0                                                   # 步长归零
                elif index - i >= 0 and tmp_place[index - i] == access[a] or tmp_place[index - i + PM] == access[a]:
                    tmp_sch.append(a)
                    change_list02(tmp_sch[-1], e, list0, list2)
                    find_same(tmp_sch, access, e, list0, list2)
                    index -= i
...
if __name__ == "__main__":
    ...
    newShift,place11 = sumShift( sch, access, PM, shift)
    result,sch11 = sumShift2(nodes, sides, sch, shift)
    print("sch", sch)
    print("sch11",sch11)
    print("place", place)

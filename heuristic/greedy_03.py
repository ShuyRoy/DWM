...
def left_neibor_position(e,list0,list2,sch,port_p,pm,shift,shift0):
    sch0 = sch[:]
    i = 0
    while True:
        i = i+1
        ...
        if port_pl >= 0:
            ...
            port_p,shift = right_neibor_position(e,sch,access,list0,list2,port_p,pm,shift,shift0)
...
def right_neibor_position(e,sch,access,list0,list2,port_p,pm,shift,shift0):
    sch0 = sch[:]
    i = 0
    while True:
        i = i + 1
...
def greedy(v,e,sch,place,port_p,pm,shift0):
    ...
    while len(sch) < len(v):
        ...
        port_p = check_position(e, sch, list0, list2, port_p, pm, place, access)
        if port_p != 0:
            port_p, shift = left_neibor_position(e, list0, list2, sch, port_p, pm, shift,shift0)
        else:
            port_p, shift= right_neibor_position(e,sch,access,list0,list2,port_p,pm,shift,shift0)
        print(sch)
        print(place)
...
def compute_max(sch,access):
    ...
    for key in count:
        i = key
        if i in shift_D:
            t = shift0[shift_D.index(i)] * count[key]
            mult_result[i] = t
    print(len(mult_result),len(shift0),len(count),mult_result)

# def improve_placement(D,sides,sch,place,shift,access,shift0,shift_D):
#     I_D,count,sort_list = count_sch(sch, access)    #I_D是访问数据的顺序  count是两个数据连续访问的次数 sort_list是连续次数降序排列
#
#     pair_list = []
#     for i in sort_list:
...
if __name__ == "__main__":
    ...
    print(len(sides))
    shift = greedy(nodes,sides,sch,place,port_p,PM,shift0)
    print("sch",sch)
    print("place",place)
    print("shift:",shift)
    print("shift0",shift0)
    ...
    #improve_placement(D,sides, sch, place, shift, access,shift0,shift_D)
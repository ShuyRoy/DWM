...
# 对应位置都有数据
def not_3(e,list0,list2,pm1,pm2,pm3,port_pt):
    for a in list0:
        if access[a] == place[port_pt + pm3] or access[a] == place[port_pt + pm2] or access[a] == place[port_pt + pm1] or \
...
# port3 所管辖范围对应位置处没有数据
def not_2(e,list0,list2,pm1,pm2,pm3,port_pt):
    for a in list0:
        ...
        elif access[a] == place[port_pt + pm3] and a not in sch:
            sch.append(a)
            change_list02(a, e, list0, list2)
        else:
            continue
...
# port2 和 port3 所管辖范围对应位置处没有数据
def not_1(e,list0,list2,pm1,pm2,pm3,port_pt):
    for a in list0:
        if access[a] == place[port_pt + pm1] or access[a] == place[port_pt] and a not in sch:
            ...
            change_list02(sch[-1], e, list0, list2)
            find_same(sch, access, e, list0, list2)
        elif access[a] == place[port_pt + pm2] or access[a] == place[port_pt+ pm3] and a not in sch:
            sch.append(a)
            change_list02(a, e, list0, list2)
        else:
            continue
...
# port1、port2 和 port3 所管辖范围对应位置处没有数据
def not_0(e,list0,list2,pm1,pm2,pm3,port_pt):
    for a in list0:
        if access[a] == place[port_pt] and a not in sch:
            ...
            change_list02(sch[-1], e, list0, list2)
            find_same(sch, access, e, list0, list2)
        elif access[a] == place[port_pt + pm1] or access[a] == place[port_pt + pm2] or access[a] == place[port_pt + pm3] and a not in sch:
            sch.append(a)
            change_list02(a, e, list0, list2)
        else:
            continue
...
# port0、port1、port2 和 port3 所管辖范围对应位置处都没有数据
def all_none(e,list0,list2,pm1,pm2,pm3,port_pt):
    for a in list0:
        if access[a] not in place and a not in sch and place[port_pt] == -1:
            ...
            change_list02(sch[-1], e, list0, list2)
            find_same(sch, access, e, list0, list2)
        elif access[a] == place[port_pt] or access[a] == place[port_pt + pm1] or access[a] == place[port_pt + pm2] or access[a] == place[port_pt + pm3] and a not in sch:
            sch.append(a)
            change_list02(a, e, list0, list2)
        else:
            continue
...
def corr_position(e,list0,list2,place,port_pt,pm1,pm2,pm3):
    #sch0 = sch[:]
    if port_pt <pm1:
        print("*****",port_pt,pm1,pm2,pm3,len(place))
        if place[port_pt+pm1] != -1 and place[port_pt+pm2] != -1 and place[port_pt+pm3] != -1:
            not_3(e,list0,list2,pm1,pm2,pm3,port_pt)    # 对应位置都有数据
        elif place[port_pt+pm1] != -1 and place[port_pt+pm2] != -1 and place[port_pt+pm3] == -1:
            not_2(e,list0,list2,pm1,pm2,pm3,port_pt)  # port3 所管辖范围对应位置处没有数据
        elif place[port_pt+pm1] != -1 and place[port_pt+pm2] == -1 and place[port_pt+pm3] == -1:
            not_1(e,list0,list2,pm1,pm2,pm3,port_pt)     # port2 和 port3 所管辖范围对应位置处没有数据
        elif place[port_pt + pm1] == -1 and place[port_pt + pm2] == -1 and place[port_pt + pm3] == -1:
            not_0(e, list0, list2, pm1, pm2, pm3,port_pt)  # port1、port2 和 port3 所管辖范围对应位置处没有数据


#返回来检查在对应位置处的指令调度完之后，是否有访问该位置处数据的指令可以调度
def check_position(e,sch,list0,list2,port_pt,pm1,pm2,pm3,place,access):
    while True:
        sch0 = sch[:]
        for a in list0:
            if access[a] == place[port_pt] and a not in sch:
...
def left_neibor_position(e,list0,list2,sch,port_pt,pm1,pm2,pm3,shift):
    sch0 = sch[:]
    i = 0
    while True:
        i = i+1
        ...
        if port_pl >= 0:
            for a in list0:
                if access[a] == place[port_pl] or access[a] == place[port_pl + pm1] or access[a] == place[port_pl + pm2] or access[a] == place[port_pl + pm3] and a not in sch:
                    sch.append(a)
                    change_list02(a, e, list0, list2)
                else:
                    continue
            ...
            port_pt,shift = right_neibor_position(e,sch,access,list0,list2,port_pt,pm1,pm2,pm3,shift)
    ...
    if port_pl < 0:
        ...
        port_pt = port_pl
    corr_position(e, list0, list2, place, port_pt, pm1,pm2,pm3)
    return port_pt,shift

def right_neibor_position(e,sch,access,list0,list2,port_pt,pm1,pm2,pm3,shift):
    sch0 = sch[:]
    i = 0
    while True:
        i = i + 1
        port_pr = port_pt + i
        if port_pr < PM1 :
            if place[port_pr] == -1 and place[port_pr + pm1] == -1 and place[port_pr + pm2] == -1 and place[port_pr + pm3] == -1:
                all_none(e,list0,list2,pm1,pm2,pm3,port_pr)
            elif place[port_pr] != -1 and place[port_pr + pm1] == -1 and place[port_pr + pm2] == -1 and place[port_pr + pm3] == -1:
                not_0(e,list0,list2,pm1,pm2,pm3,port_pr)
                corr_position(e,list0,list2,sch,port_pr,pm1,pm2,pm3)
                port_pt = check_position(e,sch,list0,list2,port_pr,pm1,pm2,pm3,place,access)
            elif place[port_pr] != -1 and place[port_pr + pm1] != -1 and place[port_pr + pm2] == -1 and place[port_pr + pm3] == -1:
                not_1(e,list0,list2,pm1,pm2,pm3,port_pr)
                corr_position(e,list0,list2,sch,port_pr,pm1,pm2,pm3)
                port_pt = check_position(e,sch,list0,list2,port_pr,pm1,pm2,pm3,place,access)
            elif place[port_pr] != -1 and place[port_pr + pm1] != -1 and place[port_pr + pm2] != -1 and place[port_pr + pm3] == -1:
                not_2(e,list0,list2,pm1,pm2,pm3,port_pr)
                remove(sch, list0)
            elif place[port_pr] != -1 and place[port_pr + pm1] != -1 and place[port_pr + pm2] != -1 and place[port_pr + pm3] != -1:
                not_3(e,list0,list2,pm1,pm2,pm3,port_pr)
                remove(sch, list0)
        else:
            break
...
def greedy(v,e,sch,place,port_p,pm1,pm2,pm3):
    ...
    while len(sch) < len(v):
        corr_position(e,list0,list2,place,port_p,pm1,pm2,pm3)
        port_p = check_position(e, sch, list0, list2, port_p, pm1,pm2,pm3, place, access)
        if port_p != 0:
            port_p, shift = left_neibor_position(e, list0, list2, sch, port_p, pm1,pm2,pm3, shift)
        else:
            port_p, shift= right_neibor_position(e,sch,access,list0,list2,port_p,pm1,pm2,pm3,shift)
        # print(sch)
        # print(place)
        print(list0)
    return shift
...
if __name__ == "__main__":
    ...
    place = [-1] * P
    port_p = 0
    shift = greedy(nodes, sides, sch, place, port_p, PM1,PM2,PM3)
